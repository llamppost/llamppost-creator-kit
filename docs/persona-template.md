...
### `agent_skills`（Agent 必填、单品选填）

如果这个 persona 要跟特定 skill 打包成一个 Agent 一起卖，把它们的 `skill_id` 列在这里。每一个列出的 skill 必须：
...
> **persona 不绑任何特定 skill。** 一个 persona ＝ 角色的人格＋工作风格（开场／回报／收尾行为）＋它的灵魂，本身就独立成立。skill 是另一个独立、可叠加的层；`agent_skills` 只标记哪些 skill 跟它**搭配得好**，并不定义这个 persona。
