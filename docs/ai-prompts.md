...
1. まず**以下のファイルの完全な内容**を会話の冒頭に貼り付け：
   ...
   - フルセット（全部）→ `bundles/YOUR_BUNDLE_NAME/` 全体（README.md + persona.md + SKILL.md + avatar/metadata.json）+ 上記の 3 セット
...
## プロンプト D：フルセット上架（Persona + Skill + Avatar）
...
- docs/listing-ready-v1.md（上架ガイド全体——特に「フルセットを公開する」セクション）
- docs/persona-template.md、docs/skill-template.md、docs/avatar-creation-spec.md
- bundles/YOUR_BUNDLE_NAME/README.md（フルセットテンプレートの使い方 + 双方向バインディング例）
- bundles/YOUR_BUNDLE_NAME/persona.md + SKILL.md + avatar/metadata.json（空白フルセットテンプレート）
...
- Persona の bundled_skills フィールドもこの Skill の skill_id を必ず含む
...
- 完全なフルセットフォルダ構造（bundles/<bundle_name>/persona.md + SKILL.md + avatar/metadata.json）
- 推奨 bundle フォルダパス（例：`bundles/night_wolf_strategist/`）
- 双方向バインディング確認（Persona.bundled_skills ↔ Skill.compatible_personas）
...
- Persona ↔ Skill の bundled_skills ↔ compatible_personas は必ず双方向バインディング
- フルセットは 3 つすべて必須（Persona + Skill + Avatar）。Avatar なしではフルセット上架不可——単品に分解するか先に Avatar 生成するか教えてください
...
- [ ] Persona + Skill バインディングの組み合わせ（フルセット）なら、双方向バインディングが両側書けている
- [ ] フルセット上架時、`avatar/avatar.png` を補ってある（placeholder だけではない）
