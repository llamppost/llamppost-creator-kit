...
言語：日本語 | [English](../../en/docs/listing-ready-v1.md) | [繁體中文](../../zh-TW/docs/listing-ready-v1.md) | [简体中文](../../zh-CN/docs/listing-ready-v1.md)
...
**この 5 セクションが公開コンテンツページになります**——商品ページにレンダリングされ、購入者が購入前に読む内容です。しっかり書いてください。
...
これらは Portal で操作します。対応する Kit ファイル形式はありません。

### 出品をいつでも編集

**Studio →「我的上架（マイ出品）」** で、公開済み商品のコンテンツページ、サムネイル、名前、説明をいつでも編集できます。変更は即時反映——再審査は不要です。

### 複数の skill をスキルパックにまとめる

**Studio →「我的上架（マイ出品）」→「建立技能包（スキルパック作成）」** で、公開済みの skill を複数まとめて 1 つの**スキルパック**にできます——それ自体が独自の価格とサムネイルを持つマーケット商品です。まとめた skill は引き続き個別に購入できます。
